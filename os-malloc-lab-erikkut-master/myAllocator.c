...
/* find the next block with usable space > s */
static BlockPrefix_t *z; //used to save value of last block found
BlockPrefix_t *findNextFit(size_t s) {    //added for lab
  if(z == NULL)
    z = arenaBegin;//start from the beginning if first time calling
  
  while (z) {
    if (!z->allocated && computeUsableSpace(z) >= s){
      return z;
    }
    z = getNextPrefix(z);
  }
...
void *nextFitAllocRegion(size_t s){                         //added for lab
  size_t asize = align8(s);
  BlockPrefix_t *p;
  if (arenaBegin == 0)		/* arena uninitialized? */
    initializeArena();
...
/* Created and used to compute the space in between two block prefixes */
size_t computeHoleSpace(BlockPrefix_t *x, BlockPrefix_t *y){  //added for lab 
  void *suffixOfFirst = x->suffix;
  
  /*  FOR DEBUGGING PURPOSES
  printf("prefixOfFirst: %x\n", x);
  printf("suffixOfFirst: %x\n", suffixOfFirst);
  printf("prefixOfSecon: %x\n", y);
  printf("suffixOfSecon: %x\n", y->suffix);
  printf("Space in between them: %x\n", (void *)y-suffixOfFirst);
  */
  return (void *)y - suffixOfFirst;
}

/* Edited to enlarge a region if possible rather than finding a bigger
   one a copying it */
void *resizeRegion(void *r, size_t newSize){                //modified for lab
  int oldSize;
  if(r != (void *)0) //checks that address isn't empty
    oldSize = computeUsableSpace(regionToPrefix(r));
  else
    oldSize = 0;
  if(oldSize >= newSize) //block is already the new size
    return r;
  else{
    if(oldSize < newSize){
      BlockPrefix_t *current = regionToPrefix(r);  //make block input addr.
      BlockPrefix_t *nxt = getNextPrefix(current);   //find next empty block
	...
	if(oldSize + forwardSpace >= newSize){
	  ...
	  makeFreeBlock(r, newSize);     //make empty block on that region

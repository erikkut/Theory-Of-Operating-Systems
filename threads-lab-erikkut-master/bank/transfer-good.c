/*  simple bank transaction demo using threads.
 ...
 */
int doWithdraw(ATM *atm, Account *account, int request){
 ...
 */
void doDeposit(ATM *atm, Account *account, int request){
 ...
 */
void transfer01(ATM *atm){
  int i;
  for (i = 0; i < 3; i++){
    mutexLock(&accounts[0].lock);//////////////////////////////////
    mutexLock(&accounts[1].lock);
    doTransfer(atm, &accounts[0], &accounts[1], 10);
    mutexUnlock(&accounts[0].lock);//////////////////////////////////
    mutexUnlock(&accounts[1].lock);
  }
} 
